...
3. **動態生成 HTML 結構**：
   ...
   * 在 Habits 頁面中，將單一習慣卡片封裝為函式（`makeHabitCard()`），動態建立多層 DOM 結構，提升程式的可重用性與結構性。

4. **基礎的資料狀態管理 (State Management)**：
   * 在 Todo 及 Habits 頁面中，我們宣告了陣列（Array）與物件（Object）來儲存任務清單（`tasks`、`habits`），並根據這些資料狀態來重新渲染 (`render()`) 畫面，實現了資料與畫面的同步。
   * 在 Habits 中進一步將資料操作邏輯模組化（如 `getHabit()`、`updateHabit()`），並透過統一的 `renderHabits()` 控制畫面更新流程。
   * 利用條件判斷（如 `completedToday`）來驅動畫面分區（In-Progress / Done），體現資料驅動畫面的設計方式。
...
4. **鍵盤事件攔截 (Keyboard Event Handling)**：
   ...
   * 在 Habits 頁面的 Modal 中，也支援 `Enter` 與 `Ctrl/Cmd + Enter` 快速提交資料，提升操作效率。
